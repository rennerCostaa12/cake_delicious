...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($qntFloors) || empty($typePasta) || empty($filling) || empty($receipt_date) || empty($freteSelect)) {
        ...
        if ($qntFloors || $typePasta || $filling || $roof || $note || $freteSelect) {
            if($isShipping){
                $sqlInsert = $pdo->prepare("INSERT INTO cakes (category_cake_id, user_id, size_cake, type_pasta, receipt_date, filling, roof, note, price, isshipping) VALUES (1, :user, :size_cake, :type_pasta, :receipt_date, :filling, :roof, :note, :price, true);");
            }else{
                $sqlInsert = $pdo->prepare("INSERT INTO cakes (category_cake_id, user_id, size_cake, type_pasta, receipt_date, filling, roof, note, price, isshipping) VALUES (1, :user, :size_cake, :type_pasta, :receipt_date, :filling, :roof, :note, :price, false);");
            }
